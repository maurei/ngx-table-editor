...
export class TypeaheadContainerComponent {
	parent: TypeaheadCellControlValueAccessor;
	query: any;
	element: ElementRef;
	isFocused = false;
	top: string;
	...
	highlight(match: TypeaheadMatch, query: any): string {
		...
		let itemStrHelper: string = (this.parent && this.parent.typeaheadLatinize ? latinize(itemStr) : itemStr).toLowerCase();
		let startIdx: number;
		let tokenLen: number;
		...
		if (this.liElements) {
			...
			if (liElement && !this.isScrolledIntoView(liElement.nativeElement)) {
				this.ulElement.nativeElement.scrollTop =
					liElement.nativeElement.offsetTop - this.ulElement.nativeElement.offsetHeight + liElement.nativeElement.offsetHeight;
			}
		}
	}

	private isScrolledIntoView = function(this: any, elem: HTMLElement) {
